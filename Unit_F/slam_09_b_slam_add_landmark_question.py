...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        G = zeros((robot_pose+landmark_states, robot_pose+landmark_states))
        G[0:robot_pose, 0:robot_pose] = G3
        G[robot_pose:, robot_pose:] = G_lower_landmark
        print(shape(G))
        R = zeros((robot_pose+landmark_states, robot_pose+landmark_states))
        R[0:robot_pose, 0:robot_pose] = R3
        ...
        self.state[0:robot_pose] = self.g(self.state[0:robot_pose], control, self.robot_width)  # Replace this.
        print(self.state[0:robot_pose])
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        covariance_new = zeros((dim_x+landmark_states, dim_y+landmark_states))
        covariance_new[0:dim_x, 0:dim_y] = self.covariance
        # add sigma for the two landmards
        landmark_sigma = diag([1e10, 1e10]) 
        covariance_new[dim_x:, dim_y:] = landmark_sigma
        self.covariance = covariance_new
        x, y = initial_coords
        landmark_array = array([x, y])
        self.state =  hstack((self.state[:,], landmark_array[:,]))
        return (-1+self.number_of_landmarks)
