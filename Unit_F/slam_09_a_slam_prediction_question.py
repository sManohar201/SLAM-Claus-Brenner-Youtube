...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        landmark_states = self.number_of_landmarks * 2
        G_lower_landmark = eye(landmark_states)
        G_upper_right = zeros((len(self.state), landmark_states))
        G_lower_left = G_upper_right.T 
        G = zeros((len(self.state)+landmark_states, len(self.state)+landmark_states))
        G[0:len(self.state), 0:len(self.state)] = G3
        G[len(self.state):, 0:len(self.state)] = G_lower_left
        G[len(self.state):, len(self.state):] = G_lower_landmark
        G[0:len(self.state), len(self.state):] = G_upper_right
        # print(G)
        R = zeros((len(self.state)+landmark_states, len(self.state)+landmark_states))
        R_lower_right = zeros((landmark_states, landmark_states))
        R_upper_right = zeros((len(self.state), landmark_states))
        R_lower_left = zeros((landmark_states, len(self.state)))
        R[0:len(self.state), 0:len(self.state)] = R3
        R[len(self.state):, len(self.state):] = R_lower_right
        R[len(self.state):, 0:len(self.state)] = R_lower_left
        R[0:len(self.state), len(self.state):] = R_upper_right
        # print(R)
