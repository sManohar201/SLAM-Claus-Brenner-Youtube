...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in range(len(cylinders)):
        # define a variable to hold min reference index
        min_ind = -1
        for j in range(len(reference_cylinders)):
            dis = calculate_dis(cylinders[i], reference_cylinders[j]) 
            if dis < max_radius:
                min_ind = j
        if min_ind > 0:
            cylinder_pairs.append((i, min_ind))
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    cos_vec, sin_vec, rr, ll = 0.0, 0.0, 0.0, 0.0

    for i in range(len(right_list)):
        r = (right_list[i][0]-rc[0], right_list[i][1]-rc[1])
        l = (left_list[i][0]-lc[0], left_list[i][1]-lc[1])
        cos_vec += r[0]*l[0] + r[1]*l[1]
        sin_vec += -r[0]*l[1] + r[1]*l[0]
        rr += r[0]*r[0] + r[1]*r[1]
        ll += l[0]*l[0] + l[1]*l[1]
    
    if (ll-0.0) < 0.0001:
        return None
    la = sqrt(rr/ll) if not fix_scale else 1 
    normal = sqrt(cos_vec**2 + sin_vec**2)
    c = cos_vec/normal if normal > 0 else 0
    s = sin_vec/normal if normal > 0 else 0
