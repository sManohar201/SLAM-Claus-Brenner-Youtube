...
def kalman_filter_step(belief, control, measurement):
    ...
    # prediction step
    mean_pred = belief.mu + control.mu
    sigma_pred = belief.sigma2 + control.sigma2
    # Prediction.
    prediction = Density(mean_pred, sigma_pred)  # Replace

    kalman_gain = sigma_pred/(sigma_pred + measurement.sigma2)

    # update mean
    mean_update = mean_pred + kalman_gain * (measurement.mu - mean_pred)
    sigma_update = (1 - kalman_gain) * sigma_pred
    # Correction.
    correction = Density(mean_update, sigma_update)# Replace
